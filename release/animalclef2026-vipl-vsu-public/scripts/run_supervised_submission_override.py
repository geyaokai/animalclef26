...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build a submission variant by overriding one dataset with a supervised checkpoint.")
    ...
    np.save(embeddings_dir / f"{dataset}_supervised_test_embeddings.npy", dataset_embeddings.astype(np.float32))
    dataset_df.to_csv(embeddings_dir / f"{dataset}_supervised_test_metadata.csv", index=False)
    ...
    for row in route_df.itertuples(index=False):
        ...
        if str(row.dataset) == dataset:
            desc = (
                f"supervised checkpoint `{args.checkpoint_path.resolve()}`, "
                f"`B x 3 x {int(checkpoint_config['input_size'])} x {int(checkpoint_config['input_size'])} "
                f"-> B x {int(checkpoint_config['student_feature_dim'])} -> B x {int(checkpoint_config['embedding_dim'])}`"
            )
        ...
        elif "texas" in route_name.lower():
            desc = "Texas-only self-trained `MiewID` student, `B x 3 x 440 x 440 -> B x 2152 -> B x 512`"
        else:
            desc = f"route `{route_name}`, embedding dim `{int(row.embedding_dim)}`"
        rerank_text = "ORB rerank enabled" if bool(row.rerank_enabled) else "no local rerank"
        architecture_lines.append(
            f"  - `{row.dataset}`: {desc}, threshold `{float(row.threshold)}`; {rerank_text}."
        )
    ...
    lines = [
        "# Supervised Submission Override",
        "",
        f"- Override dataset: `{dataset}`",
        f"- Route name: `{args.route_name}`",
        f"- Checkpoint path: `{args.checkpoint_path.resolve()}`",
        ...
        *architecture_lines,
        ...
        "",
        "- `single-dataset override`: keep `Lynx / SeaTurtle / Texas` and the current Salamander baseline frozen in the base submission, then replace only one dataset branch.",
        f"- `current changed trick`: override `{dataset}` from the previous official route to supervised `{args.route_name}` with threshold `{float(args.threshold)}`.",
        "- `path fallback safety`: if legacy manifest paths point to deleted preprocessing artifacts, the override resolves back to an existing image path before inference.",
        "",
    ]
    (reports_dir / "summary.md").write_text("\n".join(lines), encoding="utf-8")

    print(f"[supervised_submission_override] submission: {output_dir / 'submission.csv'}")
    print(f"[supervised_submission_override] predictions: {tables_dir / 'test_predictions_v1.csv'}")
    print(f"[supervised_submission_override] summary: {reports_dir / 'summary.md'}")
    return 0
