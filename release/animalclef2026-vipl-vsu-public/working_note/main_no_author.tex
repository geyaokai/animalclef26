...
The verification component of our system was inspired by ambiguity-aware pair sampling for animal re-identification \cite{sani2026active}. That work frames feedback as pairwise constraints, producing must-link and cannot-link signals for constrained clustering. We adopt the same pairwise interface in a narrower engineering form: the model proposes candidate pairs, and the verification stage only accepts or rejects pair edges. In our competition setting, these decisions are compiled into clustering constraints over top-$K$ local matches and uncertain clusters rather than used as exhaustive identity labels.
...
Open-set re-identification systems often need to convert retrieval evidence into a partition, so pair-level decisions become useful when descriptor scores alone are ambiguous. Ambiguity-aware pair sampling for animal re-identification converts selected pairwise feedback into must-link and cannot-link constraints for clustering \cite{sani2026active}. We use this line of work as the conceptual basis for treating verified candidate pairs as graph constraints.
...
The officially released data contain \num{15483} images in four dataset namespaces. The train split contains \num{13074} labelled images for three datasets, while \datasetname{TexasHornedLizards} is provided as a test-only dataset with \num{274} images. The test split contains \num{2409} images in total. The datasets are released through the AnimalCLEF benchmark \cite{animalclef2026overview}; where available, we also cite the underlying dataset or species study used by the benchmark \cite{animalclef2025overview,seaturtleid2022,czechlynx,biffi2025identification}. Submissions were handled on the official Kaggle competition page \cite{animalclef2026}.

\begin{table}[t]
  \centering
  \caption{Dataset sizes in the released AnimalCLEF 2026 package used by our system.}
  \label{tab:data}
  \begin{tabular}{lrrl}
    \toprule
    Dataset & Train images & Test images & Main visual cue / source \\
    \midrule
    \dataset{LynxID2025} & 2957 & 946 & coat/flank markings; CzechLynx-related \cite{czechlynx} \\
    \dataset{SalamanderID2025} & 1388 & 689 & dorsal yellow spot patterns \cite{animalclef2025overview} \\
    \dataset{SeaTurtleID2022} & 8729 & 500 & facial and scute patterns \cite{seaturtleid2022} \\
...
Local validation refers to our internal experiment protocol, not the official Kaggle evaluation split. For each labelled dataset, identities are shuffled with seed 42 and approximately 10\% of identities are held out; all images of the same individual stay on the same side of the split. This produces 991 fit identities with \num{11739} fit images and 111 validation identities with \num{1335} validation images. The validation split contains 69/8 fit/validation identities for lynx, 528/59 for salamander, and 394/44 for sea turtle. Official competition scores are reported separately from the Kaggle public and private leaderboards.
...
For frozen descriptor fusion, MegaDescriptor and MiewID embeddings are concatenated and L2-normalised. For learned routes, a supervised student projects the backbone output into a 512-dimensional embedding and is clustered using a dataset-specific threshold. The final submission is then assembled by replacing one dataset branch at a time, which made leaderboard changes attributable during development.
...
The official Texas horned lizard reference paper by Biffi et al. \cite{biffi2025identification} was important for this branch. It confirmed that individual identity can be read from ventral spot patterns. In their HotSpotter experiment, the first-ranked candidate was correct for 94.4\% of known recapture photos, while most remaining failures still contained the true match in the top-10 list. This directly shaped our decomposition: first build an inclusive top-$K$ candidate set, where the main objective is recall, then spend the second stage only on deciding which candidate links are trustworthy. Our pipeline therefore used SAM-aligned \cite{sam}, gray-percentile, center-body-square views before local matching and pair verification. Figure~\ref{fig:texas-preprocess-audit} shows the early data-analysis audit that led to this view-normalisation route.
...
The self-training route used pseudo-labels only after view normalization and conservative candidate filtering. We first constructed SAM-aligned gray center-body views, retrieved top-$K$ neighbours using descriptor and local evidence, and accepted only high-confidence links as trusted pseudo-positive pairs. These pairs, together with single-view consistency positives from all images, produced a small trusted training set for a Texas-specific MiewID student. The self-training results made the next bottleneck clear: the clustering graph needed more reliable positive connections. We therefore introduced pair-constraint verification to increase both the quantity and trustworthiness of graph edges. Descriptor and local-match scores produced high-recall candidate pairs; automatic rules first selected trusted pairs for self-training, and later explicit pair review handled ambiguous candidates. Figure~\ref{fig:texas-orb-pair} shows the kind of local ventral-pattern evidence used by this interface.
...
The main empirical story is simpler than the submission history. Before pair-constraint verification, the system was an automatic dataset-routed pipeline: supervised/frozen descriptors, salamander ORB and XGBoost pairwise fusion, yellow-pattern features, sea-turtle crop fusion, lynx seed smoothing, and conservative constraint compilation. The public-best version of this stage reached 0.51595 public ARI, while the strongest private score observed before pair-constraint verification was 0.46575. After introducing pair-constraint verification as a deliberate loop, first for Texas and then for salamander, the final submission reached 0.67987 public ARI and 0.69632 private ARI.

Table~\ref{tab:public-progress} summarizes the leaderboard-linked progression at the level of intervention stages. The trace should not be read as a controlled factorial ablation: it records the practical route by which the system moved from automatic clustering to a stronger graph-constrained solution.

\begin{table}[t]
  \centering
  \caption{Before/after pair-constraint verification summary. Scores are ARI. The rows summarize documented intervention stages rather than a controlled factorial ablation.}
  \label{tab:public-progress}
  \small
  \begin{tabularx}{\linewidth}{lYrr}
    \toprule
    Stage & Main change & Public ARI & Private ARI \\
    \midrule
    Before pair constraints, public-best base & automatic routed clustering with learned pairwise fusion and conservative constraint compilation & 0.51595 & 0.44839 \\
    Before pair constraints, private-best observed base & strongest private-score pre-verification submission in the Kaggle export & 0.48835 & 0.46575 \\
    After pair constraints & Texas trusted-view self-training plus salamander top-$K$ pair verification and positive graph constraints & 0.67987 & 0.69632 \\
    \bottomrule
  \end{tabularx}
\end{table}

The two before-verification rows use different selection rules. The public-best base is the route we used as the development anchor during the competition. The private-best observed base is reported only as a post-hoc reference after the private leaderboard scores and ranks were released; it was not used for competition-time model selection.

Table~\ref{tab:numerical-evidence} gives a more intuitive public-leaderboard trace before the final pair-constraint stage. The rows should be read as engineering milestones rather than isolated controlled ablations, but they show where the largest visible gains came from.

\begin{table}[t]
  \centering
  \caption{Intuitive public-leaderboard progression before the final pair-constraint stage. Scores are public ARI.}
  \label{tab:numerical-evidence}
  \small
  \begin{tabularx}{\linewidth}{lrlY}
    \toprule
    Stage & Public ARI & Gain & Main change \\
    \midrule
    ...
    Texas self-training & 0.457 & +0.085 & SAM-aligned Texas views, high-confidence top-$K$ pseudo-positive links, and a Texas-specific MiewID student. \\
    Salamander XGBoost fusion & 0.485 & +0.023 & Replaced hand-written score boosting with a learned pairwise fusion model over global similarities, branch-score differences, ORB score, inliers, and match ratios. \\
    Yellow-ROI local evidence & 0.515 & +0.030 & Pair review indicated that salamander identity depends strongly on dorsal yellow patterns; colour-based yellow ROIs were therefore used to focus the local evidence before ORB-based matching features were fused. \\
    Final pair constraints & 0.67987 & +0.164 & Candidate pair verification and positive graph constraints were scaled to Texas and salamander, producing the final submitted clustering. \\
    \bottomrule
  \end{tabularx}
\end{table}

The Texas branch provides the intermediate evidence for the pair-verification formulation: use preprocessing and local matching to create high-recall candidate pairs, verify only those pairs, and compile the results into trusted constraints rather than assigning global identity labels. Additional cluster-shape diagnostics and negative probes are reported in Appendix~\ref{app:additional-results}.
...
Another lesson is that a test-only clustering graph can have an insufficient information budget for some individuals. If the released test images provide only one weak view of an animal, aggregating neighbours within the test split may not expose enough distinctive markings to decide a merge or split. Multi-view evidence and reference-set anchoring can therefore change the problem: additional views, cropped body parts, segmentation-normalised views, or links to labelled reference identities add evidence that is not present in a single test embedding. Our system used this principle selectively: sea turtle benefited from cropped-view fusion, Texas horned lizards benefited from SAM-aligned body views, and salamanders benefited from local and yellow-pattern pair evidence. In all cases, the practical issue was not only how to cluster the test set, but how to expose enough identity information before clustering.
...
The largest limitation of this note is that it records a competition system whose final branch selection depended on public leaderboard feedback and many local artifacts. We therefore report leaderboard gains mainly as intervention-level evidence rather than as a fully controlled ablation over all possible branch combinations. For reproducibility, this note reports the branch-level preprocessing, model choices, thresholds, and graph-compilation settings used by the submitted system. The implementation was maintained in an internal team repository during the competition, and cleaned inference scripts and configuration files will be released after internal cleanup. The final submitted CSV was uploaded on 2026-04-29 03:04:38 UTC and is associated with the public/private scores reported in Table~\ref{tab:public-progress}.

\section{Conclusion}
We presented a dataset-routed hybrid clustering system for AnimalCLEF 2026. The final approach combined pretrained wildlife descriptors, supervised metric learning, local ORB evidence, XGBoost pairwise fusion, transductive seed smoothing, graph-constraint compilation, and trusted-view self-training. The system obtained 0.67987 public ARI and 0.69632 private ARI. The strongest gains came from respecting the different information structure of each dataset rather than forcing a universal route.
...
\begin{table}[h]
  \centering
  \caption{Main branch configuration for the best documented public system.}
...
\begin{table}[h]
  \centering
  \caption{Key operating parameters of the submitted system. Thresholds are cosine-distance clustering thresholds unless noted otherwise.}
